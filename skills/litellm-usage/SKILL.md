...
- `unknown provider for model <name>` on a `codex`-family model (`gpt-5.6-terra`/`gpt-5.6-luna`/`gpt-5.6-sol`/`subscription-codex*`) — **don't diff the LiteLLM deployment config first**, it's very likely identical to a sibling model_name that works fine (checked via `curl http://127.0.0.1:4000/model/info -H "Authorization: Bearer $MASTER_KEY"`). This means cliproxyapi's whole `codex` account pool (only 2 accounts: `ai2@cancerfree.io`, `ai4@cancerfree.io`; `min_enabled: 0` lets the pool go to zero) had no usable account at that moment. Check `journalctl -u cliproxy-usage-guard.service | grep -i codex` for `401`/`held`/`timeout` on both accounts around the failure time. A account stuck on repeated `401` needs a human to re-run `-codex-login` (interactive) or `-codex-device-login` (headless) on `cli-proxy-api` — it will not self-heal.
